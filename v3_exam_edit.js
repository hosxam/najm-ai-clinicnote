(function() {
  ...
  function isEnabled() {
    ...
    return (mode === "exam-edit" || mode === "workbench") && window.CLINICNOTE_DATA_MODE === "v2";
  }
  ...
  function panel() {
    return document.getElementById("v3ExamEditPanel");
  }
    ...
    fetch("./data/v3_exam_prompt_templates.json", { cache: "no-store" })
      ...
      .catch(function(error) {
        ...
        if (p && isEnabled()) {
          ...
          p.innerHTML = '<div class="v3-history-title">V3 Exam Checklist Prototype</div><div class="v3-history-safety">Exam templates could not be loaded. OPD note generation is unchanged.</div>';
        }
        console.warn("ClinicNote V3 exam checklist could not load local templates", error);
      });
  }
  ...
  function selectedLines() {
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      if (sectionLines.length) {
        lines.push(section.section_label || section.section_id || "Exam section");
  ...
  function buildDraft() {
    var template = activeTemplate();
    if (!template) return "";
    var lines = ["Exam documentation draft preview", "Specialty: " + (template.specialty_name || template.specialty_id), ""].concat(selectedLines());
    lines.push("Examination prompts are documentation support only. Select only findings you assessed.");
    return lines.join("\n").trim();
  }

  function updatePreview() {
    var box = document.getElementById("v3ExamDraftPreview");
    if (box) box.textContent = selectedLines().length ? buildDraft() : "Exam documentation draft preview will appear here as prompts are selected.";
  }
  ...
  function render() {
    ...
    if (!templates) {
      p.classList.add("v3-edit-visible");
      p.style.display = "block";
      p.innerHTML = '<div class="v3-history-title">V3 Exam Checklist Prototype</div><div class="v3-history-status">Loading exam checklist prototype...</div>';
    ...
    var html = '<div class="v3-history-header"><div><div class="v3-history-title">V3 Exam Documentation Checklist Prototype</div><div class="v3-history-subtitle">Temporary checklist only. It is not used to generate the OPD note.</div></div></div>';
    html += '<div class="v3-history-safety">Examination prompts are documentation support only. Select only findings you assessed. Document only if assessed.</div>';
    html += '<div class="v3-history-controls"><label for="v3ExamEditSpecialtySelect">Exam specialty</label><select id="v3ExamEditSpecialtySelect" onchange="window.v3ExamEditSelectSpecialty(this.value)">' + buildSpecialtyOptions(activeId) + '</select></div>';

    if (!template) {
      html += '<div class="v3-history-status">Select a workflow or choose a V3 specialty to preview checklist prompts.</div>';
    } else {
      ...
      for (var i = 0; i < sections.length; i++) {
        ...
        for (var pIndex = 0; pIndex < prompts.length; pIndex++) {
          ...
          html += '<input type="checkbox" data-v3-exam-prompt="' + escapeHtml(key) + '" onchange="window.v3ExamEditToggle(this)"' + (selectedPrompts[key] ? " checked" : "") + '>';
    ...
    html += '<div class="v3-edit-actions"><button class="btn btn-primary btn-sm" type="button" onclick="window.v3ExamEditCopy()">Copy Draft</button><button class="btn btn-outline btn-sm" type="button" onclick="window.v3ExamEditClear()">Clear Selected Prompts</button></div>';
    html += '<div class="v3-history-title">Exam documentation draft preview</div><div id="v3ExamDraftPreview" class="v3-edit-preview"></div>';
    p.classList.add("v3-edit-visible");
    p.style.display = "block";
    p.innerHTML = html;
    updatePreview();
  ...
  window.v3ExamEditSelectSpecialty = function(specialtyId) {
    selectedSpecialty = specialtyId || "";
    selectedPrompts = {};
    render();
  };

  window.v3ExamEditToggle = function(el) {
    var key = el.getAttribute("data-v3-exam-prompt");
    if (!key) return;
  ...
  window.v3ExamEditClear = function() {
    selectedPrompts = {};
    render();
  };

  window.v3ExamEditCopy = function() {
