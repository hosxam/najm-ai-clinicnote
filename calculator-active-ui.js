(function(){
  ...
  var ACTIVE_CALCULATORS = [
    ...
    { id:"cha2ds2_vasc", name:"CHA2DS2-VASc", desc:"AF stroke risk score from clinician-entered criteria.", fields:[["chf","Congestive heart failure","select",["0","1"]],["hypertension","Hypertension","select",["0","1"]],["age75plus","Age >=75 (2 points)","select",["0","1"]],["diabetes","Diabetes","select",["0","1"]],["strokeOrTia","Previous stroke / TIA / thromboembolism (2 points)","select",["0","1"]],["vascularDisease","Vascular disease (MI, PAD, aortic plaque)","select",["0","1"]],["age65to74","Age 65-74","select",["0","1"]],["female","Female sex","select",["0","1"]]] },
    { id:"has_bled", name:"HAS-BLED", desc:"Bleeding risk score for patients on anticoagulation from clinician-entered criteria.", fields:[["hypertensionUncontrolled","Uncontrolled hypertension (SBP >160)","select",["0","1"]],["abnormalRenalFunction","Abnormal renal function (Cr >=200 umol/L or dialysis)","select",["0","1"]],["abnormalLiverFunction","Abnormal liver function","select",["0","1"]],["strokeHistory","History of stroke","select",["0","1"]],["bleedingHistory","History of major bleeding or predisposition","select",["0","1"]],["labileINR","Labile INR","select",["0","1"]],["ageGT65","Age >65","select",["0","1"]],["drugsAlcohol","Drugs (antiplatelets/NSAIDs) or alcohol","select",["0","1"]],["drugsAlcoholBoth","BOTH drugs and alcohol (extra point)","select",["0","1"]]] },
    { id:"egfr_ckd_epi", name:"eGFR (CKD-EPI 2021)", desc:"Race-free CKD-EPI 2021 estimated GFR from clinician-entered creatinine, age, and sex.", fields:[["creatinine","Serum creatinine (umol/L)","number"],["age","Age (years)","number"],["female","Female sex","select",["0","1"]]] },
    { id:"news2", name:"NEWS2", desc:"National Early Warning Score 2 from clinician-entered observations.", fields:[["respiratoryRate","Respiratory rate (breaths/min)","number"],["spo2","SpO2 (%)","number"],["onSupplementalO2","On supplemental oxygen","select",["0","1"]],["temperature","Temperature (degrees C)","number"],["systolicBP","Systolic BP (mmHg)","number"],["heartRate","Heart rate (bpm)","number"],["consciousness","Consciousness","select",[["alert","Alert"],["confused","Confused / new altered mental state"]]],["hypercapnicTarget","Hypercapnic target (use SpO2 Scale 2)","select",["0","1"]]] }
  ];
  ...
  function computeHighImpact(id, f) {
    ...
    if (id === "cha2ds2_vasc" && window.calculateCHA2DS2VASc) result = window.calculateCHA2DS2VASc({chf:boolScore(f.chf,1),hypertension:boolScore(f.hypertension,1),age75plus:boolScore(f.age75plus,1),diabetes:boolScore(f.diabetes,1),strokeOrTia:boolScore(f.strokeOrTia,1),vascularDisease:boolScore(f.vascularDisease,1),age65to74:boolScore(f.age65to74,1),female:boolScore(f.female,1)});
    if (id === "has_bled" && window.calculateHASBLED) result = window.calculateHASBLED({hypertensionUncontrolled:boolScore(f.hypertensionUncontrolled,1),abnormalRenalFunction:boolScore(f.abnormalRenalFunction,1),abnormalLiverFunction:boolScore(f.abnormalLiverFunction,1),strokeHistory:boolScore(f.strokeHistory,1),bleedingHistory:boolScore(f.bleedingHistory,1),labileINR:boolScore(f.labileINR,1),ageGT65:boolScore(f.ageGT65,1),drugsAlcohol:boolScore(f.drugsAlcohol,1),drugsAlcoholBoth:boolScore(f.drugsAlcoholBoth,1)});
    if (id === "egfr_ckd_epi" && window.calculateEGFR) {
      var crVal = num(f.creatinine); var ageVal = num(f.age);
      if (!Number.isFinite(crVal) || crVal <= 0 || !Number.isFinite(ageVal) || ageVal < 0) return {ok:false,error:"Enter a positive creatinine (umol/L) and age."};
      result = window.calculateEGFR({creatinine:crVal,age:ageVal,female:boolScore(f.female,1)});
    }
    if (id === "news2" && window.calculateNEWS2) {
      var rrV = num(f.respiratoryRate); var o2V = num(f.spo2); var tV = num(f.temperature); var sbpV = num(f.systolicBP); var hrV = num(f.heartRate);
      if (!Number.isFinite(rrV) || !Number.isFinite(o2V) || !Number.isFinite(tV) || !Number.isFinite(sbpV) || !Number.isFinite(hrV) || !f.consciousness) return {ok:false,error:"Enter all NEWS2 observations and consciousness."};
      result = window.calculateNEWS2({respiratoryRate:rrV,spo2:o2V,onSupplementalO2:boolScore(f.onSupplementalO2,1),temperature:tV,systolicBP:sbpV,heartRate:hrV,consciousness:f.consciousness,hypercapnicTarget:boolScore(f.hypercapnicTarget,1)});
    }
