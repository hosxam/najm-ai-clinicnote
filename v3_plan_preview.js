(function() {
  ...
  function isEnabled() {
    ...
    return params.get("v3") === "plan" && window.CLINICNOTE_DATA_MODE === "v2";
  }
  ...
  function panel() {
    return document.getElementById("v3PlanPreviewPanel");
  }

  function hidePanel() {
    var p = panel();
    if (!p) return;
    p.classList.remove("v3-plan-visible");
  ...
  function loadTemplates(callback) {
    ...
    fetch("./data/v3_plan_prompt_templates.json", { cache: "no-store" })
      ...
      .catch(function(error) {
        var p = panel();
        if (p && isEnabled()) {
          p.classList.add("v3-plan-visible");
          p.style.display = "block";
          p.innerHTML =
            '<div class="v3-history-title">V3 Plan Documentation Prompts Preview</div>' +
            '<div class="v3-history-safety">Plan prompts could not be loaded. OPD note generation is unchanged.</div>';
        }
        console.warn("ClinicNote V3 plan preview could not load local templates", error);
      });
  }
  ...
  function renderHeader(activeSpecialty, statusText) {
    return (
      '<div class="v3-history-header">' +
        '<div>' +
          '<div class="v3-history-title">V3 Plan Documentation Prompts Preview</div>' +
          '<div class="v3-history-subtitle">Preview-only plan documentation prompts. Not used to generate the note yet.</div>' +
        '</div>' +
      '</div>' +
      '<div class="v3-history-safety">Plan documentation prompts. Use only if discussed or decided by clinician. These prompts do not recommend treatment, prescribe medication, or replace clinician judgment.</div>' +
      '<div class="v3-history-controls">' +
        '<label for="v3PlanSpecialtySelect">Preview specialty</label>' +
        '<select id="v3PlanSpecialtySelect" onchange="window.v3PlanPreviewSelectSpecialty(this.value)">' +
  ...
  function renderTemplate(template, activeSpecialty, statusText, workflowId) {
    ...
    if (!sections.length) {
      html += '<div class="v3-history-status">No plan documentation sections available in this draft template.</div>';
      return html;
    }
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      for (var p = 0; p < prompts.length; p++) {
        ...
        if (prompt.documentation_role) html += '<span class="v3-history-label">' + escapeHtml(prompt.documentation_role) + '</span>';
        html += '</div>';
  ...
  function render() {
    ...
    if (!templates) {
      p.classList.add("v3-plan-visible");
      p.style.display = "block";
      p.innerHTML = '<div class="v3-history-title">V3 Plan Documentation Prompts Preview</div><div class="v3-history-status">Loading preview prompts...</div>';
    ...
    if (currentSpecialty && template) {
      ...
      status = "Select a workflow or choose a V3 specialty to preview plan documentation prompts.";
    }
    ...
    if (!template) {
      p.innerHTML = renderHeader(activeSpecialty, currentSpecialty ? "No V3 plan prompt template available for this specialty yet." : status);
      return;
    }
  ...
  function bindFallbackEvents() {
    ...
    if (spec && !spec.getAttribute("data-v3-plan-bound")) {
      spec.setAttribute("data-v3-plan-bound", "true");
      spec.addEventListener("change", function() { setTimeout(render, 0); });
    }
    if (visit && !visit.getAttribute("data-v3-plan-bound")) {
      visit.setAttribute("data-v3-plan-bound", "true");
  ...
  window.v3PlanPreviewSelectSpecialty = function(specialtyId) {
    selectedPreviewSpecialty = specialtyId || "";
    render();
  };

  window.ClinicNoteV3PlanPreview = {
    isEnabled: isEnabled,
    render: render
  };
