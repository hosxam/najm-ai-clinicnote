(function() {
  ...
  //  V4 CHIP LOADING
  // ================================================================
  function v4IsSafeAutofillSelection(group, value) {
    if (typeof window.clinicNoteIsSafeAutofillSelection === 'function') {
      return window.clinicNoteIsSafeAutofillSelection(group, value);
    }
    var text = String(value || '').trim();
    if (!text) return false;
    if (group !== 'plan_phrases') return true;
    if (/^(?:(?:antipyretic|analgesia|symptomatic treatment|medication safety) advice|medication (?:review|adherence|safety) advice) (?:discussed|documented)(?:\.|$)|^analgesia ladder discussed(?:\.|$)/i.test(text)) return true;
    if (!/\b(?:qds|tds|bd|od|prn|nocte|stat|iv|im|sc|po|sl)\b|\/kg|\b\d+(?:\.\d+)?\s*(?:mg|mcg|ml|mL|g|%)\b|(?:\bx\s*\d+\s*(?:d|day|days|wk|wks|week|weeks)\b)|(?:×\s*\d+\s*(?:d|day|days|wk|wks|week|weeks)\b)|(?:\bfor\s*\d+\s*(?:d|day|days|wk|wks|week|weeks)\b)/i.test(text)) return true;
    if (/\b(?:medication|antibiotic|anticoagulation|analgesia|antipyretic|insulin|vaccine|vaccination|anti-d|ppi|ics|laba|lama|dapt|nsaid|steroid|inhaler|antihypertensive|diuretic|statin|sulfonylurea|antifibrotic|triple therapy|h\. pylori eradication|b12 injections)\b/i.test(text)) return false;
    return !/\b(?:paracetamol|ibuprofen|amoxicillin|phenoxymethylpenicillin|linctus|salbutamol|beclometasone|furosemide|amlodipine|ramipril|metformin|gliclazide|empagliflozin|levothyroxine|carbimazole|omeprazole|antacid|metoclopramide|ondansetron|cyclizine|loperamide|macrogol|senna|gtn|aspirin|betahistine|prochlorperazine|ferrous|cefalexin|calamine|chlorphenamine|cocp|naproxen|tranexamic|mefenamic|diclofenac|colchicine|prednisolone|amitriptyline|mometasone|dexamethasone|cyclopentolate|chloramphenicol|ofloxacin|apixaban|enoxaparin|co-amoxiclav|clarithromycin|sumatriptan|propranolol|topiramate|levetiracetam|lamotrigine|gabapentin|clopidogrel|doxycycline|gaviscon|mesalazine|mebeverine|bisacodyl|semaglutide|spironolactone|nitrofurantoin|tamsulosin|dapagliflozin|insulin|adrenaline|epinephrine|hydrocortisone|terbinafine|minoxidil|morphine|cetirizine|ranitidine|lymecycline|betamethasone|sertraline|oxymetazoline|adapalene|benzoyl|ticagrelor|cholestyramine)\b/i.test(text);
  }
        ...
        if (preset && autofillEnabled) {
          ...
          for (var pf in presetToOur) {
            ...
            for (var pi = 0; pi < preset[pf].length; pi++) {
              if (v4IsSafeAutofillSelection(g, preset[pf][pi])) {
  ...
  function stepWorkflow() {
    ...
    h += '<p class="v4-step-d">Choose a specialty, then select a workflow to load Autofill chips and Advanced Mode documentation fields. All ' + state.workflowList.length + ' workflows remain accessible.</p>';
...
function fillCalcDropdown() {
  ...
  function getCalcInputs(calcId) {
    var freq03 = [{value:'0',label:'0 = Not at all'},{value:'1',label:'1 = Several days'},{value:'2',label:'2 = More than half the days'},{value:'3',label:'3 = Nearly every day'}];
    var doze03 = [{value:'0',label:'0 = Would never doze'},{value:'1',label:'1 = Slight chance of dozing'},{value:'2',label:'2 = Moderate chance of dozing'},{value:'3',label:'3 = High chance of dozing'}];
    var ipss05 = [{value:'0',label:'0 = Not at all'},{value:'1',label:'1 = Less than 1 time in 5'},{value:'2',label:'2 = Less than half the time'},{value:'3',label:'3 = About half the time'},{value:'4',label:'4 = More than half the time'},{value:'5',label:'5 = Almost always'}];
    function fields(prefix, labels, options) {
      var out = [];
      for (var i = 0; i < labels.length; i++) out.push({key:prefix + (i + 1),label:(i + 1) + '. ' + labels[i],type:'select',options:options});
      return out;
    }
    ...
    var inputs = {
      ...
      mrc_dyspnea_scale: [{key:'grade',label:'MRC dyspnea grade',type:'select',options:[{value:'1',label:'1 = Not troubled except on strenuous exercise'},{value:'2',label:'2 = Short of breath hurrying or walking up slight hill'},{value:'3',label:'3 = Walks slower or stops when walking at own pace'},{value:'4',label:'4 = Stops after about 100m or a few minutes on level ground'},{value:'5',label:'5 = Too breathless to leave house or when dressing'}]}],
      phq_2: fields('q', phq2, freq03),
      phq_9: fields('q', phq9, freq03),
      gad_7: fields('q', gad7, freq03),
      epworth_sleepiness_scale: fields('q', epworth, doze03),
      ipss: fields('q', ipss, ipss05),
      nyha: [{key:'grade',label:'NYHA functional class',type:'select',options:[{value:'1',label:'Class I = No limitation of physical activity'},{value:'2',label:'Class II = Slight limitation of physical activity'},{value:'3',label:'Class III = Marked limitation of physical activity'},{value:'4',label:'Class IV = Symptoms at rest or unable to carry on activity'}]}],
      killip: [{key:'grade',label:'Killip class',type:'select',options:[{value:'1',label:'Class I = No clinical signs of heart failure'},{value:'2',label:'Class II = Signs of heart failure documented'},{value:'3',label:'Class III = Pulmonary edema documented'},{value:'4',label:'Class IV = Cardiogenic shock documented'}]}],
      ...
      gcs: [{key:'eyeOpening',label:'Eye opening',type:'select',options:[{value:'4',label:'4 = Spontaneous'},{value:'3',label:'3 = To speech'},{value:'2',label:'2 = To pain'},{value:'1',label:'1 = None'}]},{key:'verbal',label:'Verbal response',type:'select',options:[{value:'5',label:'5 = Oriented'},{value:'4',label:'4 = Confused'},{value:'3',label:'3 = Inappropriate words'},{value:'2',label:'2 = Incomprehensible sounds'},{value:'1',label:'1 = None'}]},{key:'motor',label:'Motor response',type:'select',options:[{value:'6',label:'6 = Obeys commands'},{value:'5',label:'5 = Localizes pain'},{value:'4',label:'4 = Withdraws from pain'},{value:'3',label:'3 = Abnormal flexion'},{value:'2',label:'2 = Extension'},{value:'1',label:'1 = None'}]}],
  ...
  function computeCalc(calcId, v) {
    ...
    function sumItems(prefix, count, min, max) {
      var sum = 0;
      for (var i = 1; i <= count; i++) {
        var n = intInRange(v[prefix + i], min, max);
    ...
    var calcFns = {
      ...
      mrc_dyspnea_scale: function() { var g = intInRange(v.grade,1,5); if (g === null) return null; var grades = ['not troubled by breathlessness except on strenuous exercise','short of breath when hurrying on level ground or walking up a slight hill','walks slower than people of the same age because of breathlessness, or stops for breath when walking at own pace','stops for breath after walking about 100m or after a few minutes on level','too breathless to leave the house, or breathless when dressing/undressing']; return { text: 'MRC dyspnea grade ' + g + ': ' + grades[g-1] + '. Documentation support only. Clinician interpretation required.', value: g }; },
      phq_2: function() { var score = sumItems('q',2,0,3); if (score === null) return null; return { text: 'PHQ-2 score: ' + score + '/6. Documentation support only. Clinician interpretation required.', value: score }; },
      phq_9: function() { var score = sumItems('q',9,0,3); if (score === null) return null; var item9 = intInRange(v.q9,0,3); var note = item9 > 0 ? ' Item 9 was marked above 0. This requires clinician review and local protocol.' : ''; return { text: 'PHQ-9 score: ' + score + '/27. Documentation support only. Clinician interpretation required.' + note, value: score }; },
      gad_7: function() { var score = sumItems('q',7,0,3); if (score === null) return null; return { text: 'GAD-7 score: ' + score + '/21. Documentation support only. Clinician interpretation required.', value: score }; },
      epworth_sleepiness_scale: function() { var score = sumItems('q',8,0,3); if (score === null) return null; return { text: 'Epworth Sleepiness Scale score: ' + score + '/24. Documentation support only. Clinician interpretation required.', value: score }; },
      ipss: function() { var score = sumItems('q',7,0,5); if (score === null) return null; return { text: 'IPSS score: ' + score + '/35. Documentation support only. Clinician interpretation required.', value: score }; },
      nyha: function() { var g = intInRange(v.grade, 1, 4); if (g === null) return null; var labels = {1:'Class I: No limitation of physical activity.',2:'Class II: Slight limitation of physical activity.',3:'Class III: Marked limitation of physical activity.',4:'Class IV: Symptoms at rest or unable to carry on physical activity without discomfort.'}; return { text: 'NYHA functional class: ' + labels[g] + ' Documentation support only. Clinician interpretation required.', value: g }; },
      killip: function() { var g = intInRange(v.grade, 1, 4); if (g === null) return null; var labels = {1:'Class I: No clinical signs of heart failure.',2:'Class II: Signs of heart failure documented.',3:'Class III: Pulmonary edema documented.',4:'Class IV: Cardiogenic shock documented.'}; return { text: 'Killip class: ' + labels[g] + ' Documentation support only. Clinician interpretation required.', value: g }; },
      ...
      gcs: function() { try { if (typeof window.calculateGCS !== 'function') return null; var g = {eyeOpening:intInRange(v.eyeOpening,1,4),verbal:intInRange(v.verbal,1,5),motor:intInRange(v.motor,1,6)}; if(g.eyeOpening===null||g.verbal===null||g.motor===null) return null; var r = window.calculateGCS(g); return r ? {text: r.interpretation + ' ' + r.safetyNotice, value: r.score} : null; } catch(e) { return null; } },
...
.v4-stepper{display:flex;gap:0;margin-bottom:20px;background:var(--bg-surface,#111c2e);border:1px solid rgba(148,163,184,.08);border-radius:var(--radius-lg);overflow:hidden}
...
.v4-sidebar-inner{background:var(--bg-surface,#111c2e);border:1px solid rgba(148,163,184,.08);border-radius:var(--radius-lg);padding:20px}
...
.v4-step-content{background:var(--bg-surface,#111c2e);border:1px solid rgba(148,163,184,.08);border-radius:var(--radius-lg);padding:24px;min-height:300px}
...
.v4-select{width:100%;padding:12px 14px;border:1px solid rgba(148,163,184,.12);border-radius:8px;font-size:14px;font-family:var(--font);background:var(--bg-elevated,#17233a);color:var(--text-primary,#f8fafc)}
.v4-select:focus{outline:none;border-color:var(--primary);box-shadow:0 0 0 3px var(--primary-glow)}
.v4-wf-info{margin-top:14px;padding:14px;background:var(--bg-elevated,#17233a);border-radius:8px}
...
/* Chip Groups (Step 1) */
.v4-chip-group{border:1px solid rgba(148,163,184,.08);border-radius:8px;margin-bottom:8px;overflow:hidden}
.v4-chip-group-header{display:flex;align-items:center;gap:8px;padding:10px 14px;background:var(--bg-elevated,#17233a);cursor:pointer;font-size:13px;font-weight:600;color:var(--text-secondary,#cbd5e1)}
.v4-chip-group-header:hover{background:var(--bg-hover,#1d2a44)}
...
.v4-chip-btn{padding:6px 14px;border-radius:20px;font-size:12px;font-weight:500;border:1px solid rgba(148,163,184,.12);background:var(--bg-elevated,#17233a);color:var(--text-secondary,#cbd5e1);cursor:pointer;font-family:var(--font);transition:all .15s;box-shadow:none}
...
.v4-hist-fields{margin-bottom:12px;padding:14px;background:var(--bg-elevated,#17233a);border-radius:8px}
...
.v4-hist-preview{background:var(--bg-elevated,#17233a);border:1px solid rgba(148,163,184,.08);border-radius:8px;padding:12px 14px;font-size:13px;line-height:1.6;white-space:pre-wrap;color:var(--text-secondary,#cbd5e1);min-height:40px}
...
.v4-calc-card{background:var(--bg-surface,#111c2e);border:1px solid rgba(148,163,184,.08);border-radius:10px;padding:14px;box-shadow:none}
...
.v4-out-tab{padding:10px 18px;border-radius:8px;font-size:13px;font-weight:600;border:1px solid rgba(148,163,184,.12);background:var(--bg-elevated,#17233a);color:var(--text-secondary,#cbd5e1);cursor:pointer;font-family:var(--font);transition:all .15s}
...
.v4-output-box{background:#060A12;color:var(--text-primary,#f8fafc);border:1px solid rgba(20,184,166,.15);border-radius:10px;padding:20px;min-height:250px;max-height:600px;overflow:auto;margin-bottom:12px}
...
.v4-nav{display:flex;align-items:center;justify-content:space-between;margin-top:16px;background:var(--bg-surface,#111c2e);border:1px solid rgba(148,163,184,.08);border-radius:var(--radius-lg);padding:12px 20px}
...
.v4-btn-outline{background:transparent;color:var(--primary);border:2px solid var(--primary)}
...
.v4-guidance{background:var(--bg-elevated,#17233a);border:1px solid rgba(148,163,184,.08);border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:12px;color:var(--text-secondary,#cbd5e1);line-height:1.6;text-align:center}
.v4-guidance-step{font-weight:600;color:var(--gray-700)}
.v4-guidance-arrow{color:var(--gray-400);margin:0 6px}
.v4-search-input{width:100%;padding:10px 14px;border:1px solid rgba(148,163,184,.12);border-radius:8px;font-size:13px;font-family:var(--font);margin-bottom:8px;background:var(--bg-elevated,#17233a);color:var(--text-primary,#f8fafc)}
